                ...
                if len(parts) == 16 and parts[0] != "mge_no":  # Skip header line and ensure proper format
                    try:
                        mge_record = (
                            ...
                            parts[3],       # prediction
                            parts[4],       # type
                            int(parts[5]),  # allele_len
                            float(parts[6]),# depth
                            float(parts[7]),# e_value
                            float(parts[8]),# identity
                            float(parts[9]),# coverage
                            int(parts[10]), # gaps
                            int(parts[11]), # substitution
                            parts[12],      # contig
                            int(parts[13]), # start
                            int(parts[14]), # end
                            parts[15]       # cigar
...
directory = '/Users/juanjovel/OneDrive/jj/UofC/data_analysis/sylviaCheckley/alyssaButters/eColi_genomics/SQLite_database/mobileelementfinder/FINAL_DATASET'
